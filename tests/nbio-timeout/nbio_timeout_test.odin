...
//
// The other three transport fixes in 44bdcda are races, and this suite does not
// cover them. It used to point at `experiments/23-accept-stall` and
// `experiments/24-shutdown-race` for that; the pointer was WITHDRAWN on
// 2026-07-31 because those harnesses are not in this repository. They were
// excluded through `.git/info/exclude`, which is per-clone and unversioned, so
// they existed in one working copy and nowhere else — nobody who cloned this
// could run the evidence this comment named, and nothing reported that. (They
// had also stopped compiling, still importing `uruquim:web` from before the
// rename to Druse.)
//
// So the honest statement is the plain one: THOSE THREE FIXES REST ON REVIEW,
// not on a committed harness. If one is wanted, it gets committed first, and
// then this comment may point at it again.
