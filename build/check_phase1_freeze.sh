...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 78 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 78 (… plus C2 set_header/bytes, plus C3 query_int_opt)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 80 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 80"
