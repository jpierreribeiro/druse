...
| P4-3 | Per-server state replacing the globals | ADR-018; prerequisite for two servers or any embedded use — **DONE. WP43 removed `g_config` (2026-07-21); WP123 removed `g_server` (2026-08-01, Amendment 44), replacing it with a generation-keyed registry of live servers. `web.stats` and `web.refused_connections` now take the App, which is the G-09 payment this row anticipated. Proven by `tests/wp123-two-servers`: two servers in one process, reporting and draining independently** |
