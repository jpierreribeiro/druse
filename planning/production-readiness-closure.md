...
| **C-06** | The reverse-proxy contract | TESTS + DOCS | ✅ **FAST FIXTURE + R1 REAL CADDY ROUND.** `planning/closure-proxy-contract.md` + `tests/c06-proxy-contract`, gated by `build/check_c06_controls.sh`. The fixture keeps buffering/trusted-hop behavior cheap on every gate. R1-WP03 adds Caddy 2.11.4 pinned by digest/platform, TLS/H2→H1.1, bounded pooling, real buffering control, two-layer limits, XFF chain/spoof, saturation/recovery, readiness-aware shutdown and redacted correlated logs through `ops/verification/run-real-proxy-contract.sh`. The evidence applies only to the reviewed Caddy topology; another proxy or materially different chain needs its own arm. |
