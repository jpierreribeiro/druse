...
| OQ-35 — **CLOSED 2026-07-31: the instrument was replaced, not the threshold.** The suite now asserts on LIVE ALLOCATOR BYTES (0 retained over the small phase, against a 512 KiB bound) and keeps RSS as a printed, unasserted figure — the treatment phase 1 already had. `build/check_c04_controls.sh` gained a negative control: each small response body is made to come from the connection allocator instead of the per-request arena, and the suite must go red. That leak is invisible to the old RSS assertion, which on this machine sees RSS *fall* by ~12 MiB across the same phase — so the previous instrument would have passed a mutant retaining megabytes. | **`tests/c04-response-size`'s RSS threshold was inside its own cross-machine noise, and CI proved it.** `LEAK_THRESHOLD_BYTES` is a fixed `2 * 1024 * 1024`, justified in prose ("leaves room for allocator bookkeeping and test-side buffers") but not derived from anything. Measured on the same commit, same code: this workstation reports **−11.9 MiB** growth over the small phase across eight runs (RSS *falls*, as the allocator returns the large-phase pages), a GitHub shared runner reports **+2.04 MiB**. That is a **14 MiB spread against a 2 MiB threshold** — the instrument cannot distinguish a leak from a machine, which is what `planning/diagnosability.md` §3 forbids for a threshold on a continuous measurement, and what `check_phase4_freeze.sh` already refuses for performance claims. It failed run 30669537434 (workflow_dispatch) and passed run 30668485510 (pull_request) on identical content. NOT caused by branch `docs/registry-reconciliation`: its only `vendor/` change is `VENDOR.md`, and it touches no `web/` code at all. | **do not raise the threshold** — that hides the noise rather than measuring. Either derive the tolerance from the machine, as `check_wp26_bench.sh` does, or stop measuring RSS: `tests/m9-attribution` was built on exactly this reasoning ("RSS cannot distinguish a live allocation from pages a process allocator has already freed but not returned to the kernel") and wraps the allocator instead. The second is the real fix. | before any release claim rests on c04, and before this instrument is cited as evidence of no leak | yes — it changes what a shipped test measures |
| OQ-34 | **A cluster of real work is excluded from the repository through `.git/info/exclude`, and the tree cites it as though it were there.** That file is per-clone and unversioned, so everything in it exists in one working copy and nowhere else: `tests/nbio-timeout` (the audit-B4 regression guard, three `@(test)`), `experiments/23-accept-stall`, `experiments/24-shutdown-race`, `ops/campaign/`, `artifacts/`, and — most consequentially — **`planning/verification-campaign-plan.md`**, which is where the rule "every reported number must be regenerable from a committed script" is actually written. Found 2026-07-31 by breaking it: `nbio-timeout` was wired into `build/check.sh`, the gate stayed green locally and would have failed on every fresh clone and in CI. Reverted, and `build/check_suite_inventory.sh` now refuses any suite the gate names that git does not track. | decide per item: commit it, or accept that it is a working note and stop citing it from tracked files. The rule text in particular should live somewhere tracked, because five tracked documents now restate it inline rather than point at it. | before the next claim that rests on campaign evidence | **yes** — `.git/info/exclude` is the owner's local file and its contents are the owner's call |
| OQ-33 — **CLOSED 2026-07-31: the citation was withdrawn** (owner decision). The harnesses stay out of the repository; `tests/nbio-timeout` no longer points at them and now states plainly that those three transport fixes rest on review rather than on a committed harness. Committing a harness later reopens nothing — it just earns the pointer back. | **Two stress harnesses cited as evidence are not in the repository.** `tests/nbio-timeout` names `experiments/23-accept-stall` and `experiments/24-shutdown-race` as the evidence path for three of the four transport fixes in `44bdcda` ("races [that] need the stress harnesses"). Both are excluded through `.git/info/exclude`, which is **per-clone and unversioned** — they exist in whichever working copy made them and nowhere else, so nobody who clones this repository can run the evidence it cites, and nothing reported that. Found 2026-07-31, together with the fact that both had also stopped compiling: they still imported `uruquim:web`, the collection name from before the rename to Druse. | commit them, or drop the citation and state plainly that those three fixes rest on review rather than on a harness. Do not leave a pointer to something only one machine has. | before any claim that the `44bdcda` transport fixes carry executable evidence | **yes** — it is a question about what the repository asserts it can prove |
